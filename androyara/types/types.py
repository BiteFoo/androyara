    ...
    def _decode8(self, offset):
        ...
        if self.m_charbuff[offset + encoded_bytes] != 0:
            ...
            raise ResParserError(
                "UTF-8 String is not null terminated! At offset={} .try to use aapt dump xmlstrings a.apk AndroidManifest.xml".format(offset))
...
class AXMLParser:
    ...
    def __init__(self, raw_buff):
        ...
        if self.filesize > self.buff.size():
            log.error("This does not look like an AXML file. Declared filesize does not match real size: {} vs {}".format(
        ...
        if header.header_size != 0x1C:
            log.error("This does not look like an AXML file. String chunk header size does not equal 28! header size = {}".format(
                header.header_size))
            self._valid = False
            return
    ...
    def _do_next(self):
        ...
        while self._valid:
            ...
            if h.type < RES_XML_FIRST_CHUNK_TYPE or h.type > RES_XML_LAST_CHUNK_TYPE:
                ...
                # subtract them.
                log.error("Not a XML resource chunk type: 0x{:04x}. Skipping {} bytes".format(
                    h.type, h.size))
            ...
            if h.type == RES_XML_START_NAMESPACE_TYPE:
                ...
                log.debug("Start of Namespace mapping: prefix {}: '{}' --> uri {}: '{}'".format(
    ...
    def getAttributeName(self, index):
        ...
        if not res:
            ...
            if attr in publics.SYSTEM_RESOURCES['attributes']['inverse']:
                res = 'android:' + \
                    publics.SYSTEM_RESOURCES['attributes']['inverse'][attr]
            else:
...
def format_value(_type, _data, lookup_string=lambda ix: "<string>"):
    ...
    def fmt_int(x): return (0x7FFFFFFF & x) - \
        0x80000000 if x > 0x7FFFFFFF else x
    ...
    def __init__(self, raw_buff):
        ...
        while self.axml.is_valid():
            ...
            if _type == START_TAG:
                ...
                log.debug("START_TAG: {} (line={})".format(
                ...
                for i in range(self.axml.getAttributeCount()):
                    ...
                    log.debug("found an attribute: {}{}='{}'".format(
            ...
            if _type == END_TAG:
                ...
                tag = "{}{}".format(uri, name)
                if cur[-1].tag != tag:
                    log.warning("Closing tag '{}' does not match current stack! At line number: {}. Is the XML malformed?".format(
...
class ARSCParser:
    ...
    def __init__(self, raw_buff):
        ...
        log.debug("Parsed ResTable_header with {} package(s) inside.".format(
            self.packageCount))
        ...
        while self.buff.get_idx() <= self.header.end - ARSCHeader.SIZE:
            ...
            if res_header.type == RES_STRING_POOL_TYPE:
                ...
                if self.stringpool_main:
                    ...
                    log.debug("Found the main string pool: %s",
                ...
                if next_idx != self.buff.tell():
                    ...
                    log.error("next_idx = {}, current buffer position = {}".format(
            ...
            if ate.is_complex():
                ...
                for _, item in ate.item.items:
                    self.put_item_value(complex_array, item,
                                        config, ate, complex_=True)
            else:
                self.put_item_value(result, ate.key, config,
                                    ate, complex_=False)
        ...
        * `hdpi-v4`
        ...
        if (screenLayout & MASK_SCREENSIZE) != SCREENSIZE_ANY:
            if screenLayout & MASK_SCREENSIZE == SCREENSIZE_SMALL:
                ...
                res.append("screenLayoutSize_%d" %
        ...
        if (screenLayout & MASK_SCREENLONG) != 0:
            if screenLayout & MASK_SCREENLONG == SCREENLONG_NO:
                ...
                res.append("screenLayoutLong_%d" %
